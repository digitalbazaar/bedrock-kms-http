...
async function getKeystoreController({req}) {
  ...
  `req.ezcap.invocationParameters.capability` to ensure that either:

  1. The `capability` is the root zcap and the verification method ID or
    controller match the keystore controller.
  2. The `capability` is delegated and the controller of the first delegated
    zcap in the chain is the keystore controller.
  ...
  let getFreshConfigRecord = false;
  ...
  if(isRoot && req.ezcap.invocationParameters) {
    ...
    const {signature: {params: {keyId}}} = req.ezcap;
    const {verificationMethod} = await getVerifier({keyId, documentLoader});
    getFreshConfigRecord =
      !(verificationMethod.controller === req.webkms.keystore.controller ||
      verificationMethod.id === req.webkms.keystore.controller);
  } else {
    const second = capability?.proof?.capabilityChain[1];
    if(second && second.controller !== req.webkms.keystore.controller) {
      getFreshConfigRecord = true;
    }
  }

  if(getFreshConfigRecord) {
    const configRecord = await storage.get({
