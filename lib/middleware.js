...
async function getKeystoreController({req}) {
  ...
  if((now - req.webkms.configRecord.meta.created) > FIVE_MINUTES ||
    ...
    logger.debug(
      'TEST - created not old enough or missing params', {
        now,
        created: req.webkms.configRecord.meta.created,
        zcapToRevoke: req.ezcap.capabilityToRevoke,
        invocationParameters: req.ezcap.invocationParameters
      });
  ...
  logger.debug('TEST - got VM', {verificationMethod});
  ...
  if(!(verificationMethod.controller === req.webkms.keystore.controller ||
    ...
    logger.debug('TEST - DID NOT BUST CACHE', {
      verificationMethod, keystoreController: req.webkms.keystore.controller
    });
  }
