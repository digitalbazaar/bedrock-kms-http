...
function _getIp(req) {
  // if express has been configured to trust a proxy req.ip will return the ip
  // @see https://expressjs.com/en/guide/behind-proxies.html
  let ip = req.ip;
console.log('GET IP CALLED');
console.log({ip, forward: req.header('x-forwarded-for'), remote: req.connection.remoteAddress});
  // if nginx forwarded the request x-forwarded-for will get the ip
  ip = ip || req.header('x-forwarded-for');
  // finally if not behind nginx or another reverse proxy then
  // connection.remoteAddress will return the ip
  return ip || req.connection.remoteAddress;
}

function _verifyRequest(url) {
