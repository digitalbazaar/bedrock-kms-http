...
// FIXME: Remove the need to specify driver mode
// FIXME: is V1 support needed?
...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  // update keystore config, also host application-based key recovery
  app.post(
    `${routes.keystore}`,
    // Note: No CORS! Must be done on the host site directly.
    _authorizeMyZcapInvocation(),
    ...
    asyncHandler(async (req, res) => {
      ...
      // FIXME: re-implement this check, does not properly address case
      // where `invoker` is not defined and there is only a controller
      /*
      ...
      */
      ...
      // FIXME: is dealing with invoker/delegator necessary?
      /*
...
async function getCapabilityController({req}) {
  const capabilityId = `${config.server.baseUri}${req.url}`;
  ...
  if(!capabilityId) {
    throw new Error('Root capability identifier could not be determined ' +
      + `for URL "${req.url}".`);
  }

  // get associated capability controller from database
  try {
    const {config} = await keystores.get({id: capabilityId});
...
function _authorizeMyZcapInvocation({expectedTarget, expectedAction} = {}) {
  return asyncHandler(authorizeZcapInvocation({
    expectedHost: config.kms.allowedHost,
    getCapabilityController,
    // FIXME: is _createWrappedDocumentLoader required? If so, additional
...
const capabilityIdRegex = new RegExp('\/keystores\/([^/]+).*');
function _createWrappedDocumentLoader({
  // FIXME: discuss how this might work. What's the use case? How can these
  // values be known when bootstrapping the route?
  ...
  return async url => {
    ...
    if(url.startsWith('urn:zcap:root:')) {
      ...
      const invocationTarget = `${config.server.baseUri}${parsedUrl.pathname}`;
    ...
    // FIXME: is anything like this needed here?
    /*
    // dynamically generate zcap for root capability if applicable
