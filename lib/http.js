...
const {generateRandom, validateRequest} = require('webkms-switch');
...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      _verifyRequest({keystoreId: id, req});
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      for(const [host] of config.allowedHosts) {
        ...
        verified = invoker.includes(`https://${host}`);
      }
      if(!verified) {
        throw new BedrockError('Permission denied.', 'NotAllowedError', {
          httpStatusCode: 400,
...
async function _handleOperation({req, expectedRootCapability, keystoreId}) {
  ...
  if(!verified) {
    throw new BedrockError('Permission denied. Expected an allowedHost but ' +
      `received "${host}".`, 'NotAllowedError', {
      httpStatusCode: 400,
      public: true,
      allowedHosts,
      host
  ...
  // if the request host is in allowedHosts use it
  const expectedHost =
    config.kms.allowedHosts.get(host) ? host : config.kms.allowedHost;
...
function _verifyRequest({keystoreId, req}) {
  const {allowedHosts} = config.kms;
  const parsed = new URL(keystoreId);
  const requestHost = req.get('host');
  const keystoreHost = parsed.host;
  // if the req's host and the keystoreId host do not match reject
  // the request
  if(requestHost !== keystoreHost) {
    throw new BedrockError('Permission denied. Expected host ' +
    `"${keystoreHost}" but received "${requestHost}".`, 'NotAllowedError', {
      httpStatusCode: 400,
      public: true,
      requestHost,
      keystoreHost
  ...
  if(!verified) {
    throw new BedrockError('Permission denied. Expected an allowedHost but ' +
      `received "${requestHost}".`, 'NotAllowedError', {
      httpStatusCode: 400,
      public: true,
      allowedHosts,
      keystoreHost
