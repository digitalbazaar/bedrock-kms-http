...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      // TODO: check `config.kms.allowedHosts` list as sanity check (user
      // sets this anyway)
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      const result = await _handleOperation({req, expectedRootCapability});
      res.json(result);
    }));
...
async function _handleOperation({req, expectedRootCapability}) {
  const host = req.get('host');
  ...
  const result = await validateOperation({
    url, method, headers,
    operation,
    expectedHost: config.kms.allowedHosts,
...
function _getInvocationTarget({host, url}) {
  ...
  // `/kms/keystores/<dataHubId>/keys`
  // `/kms/keystores/<dataHubId>/authorizations`
