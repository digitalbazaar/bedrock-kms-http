...
// TODO: split into a separate module and use bedrock-kms plugin infrastructure
...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      // TODO: implement plugins at bedrock-kms layer
      if(plugin !== 'bedrock-ssm') {
        throw new BedrockError(
          `Plugin "${plugin}" not supported.`,
          'NotSupportedError', {plugin, public: true, httpStatusCode: 400});
      }

      // ensure plugin exists and supports operation method
      const pluginApi = ssm;//this.plugins.get(plugin);
      if(!(pluginApi && typeof pluginApi[method] === 'function')) {
        throw new BedrockError(
          `Plugin method "${method}" not supported.`,
          'NotSupportedError',
          {plugin, method, public: true, httpStatusCode: 400});
      }

      const result = await pluginApi[method]({...parameters, controller});
      res.json(result);
    }));
});
