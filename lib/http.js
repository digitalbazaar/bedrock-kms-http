  ...
  app.post(
    ...
    _authorizeZcapInvocation({
      async getExpectedTarget({req}) {
        ...
        // ensure keystore can be retrieved (do IP allow list checks, etc.)
        await storage.get({id: keystoreId, req});
        // allow target to be root keystore *or* zcap-specific revocation
        // endpoint; see `_getRevocationRootController` for more details
        const revokeEndpoint = `${keystoreId}/revocations/` +
          encodeURIComponent(req.params.zcapId);
        return {expectedTarget: [keystoreId, revokeEndpoint]};
      },
