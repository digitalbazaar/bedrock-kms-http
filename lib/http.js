...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      let ctx;
      try {
        span = tracer.startSpan(`kms-operation-v3-2`, {
          attributes: {
            'span.kind': 'server',
            'service.name': 'bedrock-kms-http',
            'cloud.region': process.env.CLOUD_REGION
          }
        });
        ctx = api_1.trace.setSpan(api_1.context.active(), span);
      } catch(error) {
        logger.error('Open Telemetry Error', {error});
      }
      // const result = await _handleOperation({req});
      const result = await api_1.context.with(
        ctx, _handleOperation, undefined, {req});
...
async function _handleOperation({req, expectedRootCapability, keystoreId}) {
  ...
  let ctx;
  try {
    span = tracer.startSpan(`validate-kms-operation`);
    ctx = api_1.trace.setSpan(api_1.context.active(), span);
  } catch(error) {
    logger.error('Open Telemetry Error', {error});
  }
  const result = await api_1.context.with(ctx, validateOperation, undefined, {
    url, method, headers,
    operation,
  ...
  try {
    span = tracer.startSpan(`run-kms-operation`);
    ctx = api_1.trace.setSpan(api_1.context.active(), span);
  } catch(error) {
    logger.error('Open Telemetry Error', {error});
  }

  const opResult = await api_1.context.with(ctx, runOperation, undefined, {
    operation
  });
