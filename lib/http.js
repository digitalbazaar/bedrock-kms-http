...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  // create `suiteFactory` hook for creating suites for verifying zcap
  // delegation chains
  const suiteFactory = () => new Ed25519Signature2020();
  ...
  const handleOperation = createOperationMiddleware({
    ...
    onSuccess: reportOperationUsage, onError,
    suiteFactory
  });
  ...
  app.post(
    ...
    authorize({
      async getExpectedValues() {
        ...
        return {
          host: expectedHost,
          // expect root invocation target matching this route; this root zcap
  ...
  app.post(
    ...
    authorizeZcapRevocation({
      ...
      getRootController: _getKeystoreController,
      getVerifier,
      inspectCapabilityChain: helpers.inspectCapabilityChain,
      onError,
      suiteFactory
...
async function _getKeystoreController({
  req, rootCapabilityId, rootInvocationTarget
}) {
  const kmsBaseUrl = req.protocol + '://' + req.get('host') +
    config['kms-http'].routes.basePath;

  // FIXME: new zcapld ensures this will never happen, only expected root
  // invocation targets will result in `getRootController` getting called
  ...
  // a cached config must exist and match for the request URL
  ...
  // FIXME: new zcapld ensures this will never happen, only expected root
  // invocation targets will result in `getRootController` getting called
  // this is an invalid use of `_getRootController`
...
function authorize({
  getExpectedValues = _getKeystoreRouteExpectedValues,
  getRootController = _getKeystoreController,
  onError
} = {}) {
...
async function _getKeystoreRouteExpectedValues({req}) {
  return {
