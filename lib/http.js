...
bedrock.events.on('bedrock-kms-http.configure.routes', app => {
  ...
  app.post(
    ...
    _authorizeZcapInvocation({
      getExpectedTarget: async ({req}) => {
        const {body: {id: configId}, namespace: {id: namespaceId}} = req;
        const requestUrl = `${namespaceId}${cfg.routes.basePath}${req.url}`;
  ...
  /**
   * Invokes a generate key KMS operation to generate a new key.
   */
  app.post(
    routes.keys,
