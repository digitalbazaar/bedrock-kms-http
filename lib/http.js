...
const {BedrockKmsModuleManager, MongoDBKeyDescriptionStorage} =
  require('bedrock-kms');
const {config, util: {BedrockError}} = bedrock;
const {createMiddleware} = require('bedrock-passport');
const {runOperation, validateOperation} = require('web-kms-switch');
...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      try {
        await validateOperation(req.body);
      } catch(e) {
        throw new BedrockError(
          'Validation Error', 'ValidationError', e, {
            httpStatusCode: 400,
            public: true
          });
      }
    ...
    asyncHandler(async (req, res) => {
      ...
      const storage = new MongoDBKeyDescriptionStorage();
      const moduleManager = new BedrockKmsModuleManager();
      const result = await runOperation({operation, storage, moduleManager});
      res.json(result);
    }));
});
