...
meters.setAggregator({serviceType: SERVICE_TYPE, _aggregateUsage});
...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  async function reportOperationUsage({req}) {
    ...
    meters.use({id, usage: {operations: 1}}).catch(
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      const hasAvailable = await meters.hasAvailable(
        {meterCapability, usage: {storage: cfg.storageCost.keystore}});
      if(!hasAvailable) {
      ...
      // add meter
      const {meter: {id: meterId}} = await meters.add(
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      // FIXME: need to pass meter ID as `meta` so revocations can be
      // aggregated... or some other mechanism needs to be implemented to
      // meter stored revocations
...
async function _aggregateUsage({meter, signal} = {}) {
  // FIXME: query to get keystore IDs with matching meter ID ... and build
  // in a limit and a loop

  // FIXME: pass chunks of keystore IDs to kms modules to get key statistics
  // based on keystore IDs

  // FIXME: concurrently get total revocations with matching meter ID from
  // zcap storage API; might need to get a total revocation size? or cap
  // individual revocation size

  // FIXME: once totally aggregated, return result
}
