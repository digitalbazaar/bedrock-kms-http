...
async function _handleOperation({req, expectedRootCapability, keystoreId}) {
  ...
  const result = await validateOperation({
    ...
    documentLoader: createCapabilityLoader({host, expectedTarget: url})
  });
  if(!result.valid) {
...
async function _authorize({
  ...
  const result = await verifyCapabilityInvocation({
    url, method, headers,
    getInvokedCapability: createGetInvokedCapability(host),
    documentLoader: createCapabilityLoader({host, expectedTarget}),
...
/**
 * Gets an expectedRootCapability from an invocationTarget.
 *
 * @param {object} options - Options to use.
 * @param {string} options.invocationTarget - An
 *   invocationTarget.
...
function _getExpectedRootCapability({invocationTarget, keystoreId}) {
  // return the rootCapability for authorizations
  if(invocationTarget.endsWith('authorizations')) {
    return `${keystoreId}/zcaps/authorizations`;
  }
  // return the rootCapability for new key generation
  if(invocationTarget.endsWith('keys')) {
    return `${keystoreId}/zcaps/keys`;
  }
  // this will succeed if the target is key
...
function createCapabilityLoader({host, expectedTarget}) {
