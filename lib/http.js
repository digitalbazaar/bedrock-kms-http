...
bedrock.events.on('bedrock.init', async () => {
  const cfg = bedrock.config['kms-http'];
  if(!cfg.allowedIps) {
    throw new Error('"bedrock.config.kms-http.allowedIps" must be set.');
  }
  if(Array.isArray(cfg.allowedIps)) {
    const notString = cfg.allowedIps.some(ip => typeof ip !== 'string');
    if(notString) {
      throw new Error(
        'All entries in "bedrock.config.kms-http.allowedIps" must be strings.');
    }
  }
});
